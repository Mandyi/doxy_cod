...
#define portINTERRUPT_BITS			( ( unsigned portSHORT ) 7/*configKERNEL_INTERRUPT_PRIORITY*/ << ( unsigned portSHORT ) 5 )

//#define portDISABLE_INTERRUPTS()	SR |= portINTERRUPT_BITS                    
//#define portENABLE_INTERRUPTS()		SR &= ~portINTERRUPT_BITS

#define portDISABLE_INTERRUPTS()	asm volatile ("disi #0x3FFF");
#define portENABLE_INTERRUPTS()		asm volatile ("disi #0x0");
...
#define portYIELD_ISR()				asm volatile ( "CALL _vPortYieldISR		\n"		\
												   "NOP					  ");
/*-----------------------------------------------------------*/
